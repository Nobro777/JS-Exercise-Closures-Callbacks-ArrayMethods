...
function processSum (numberList, callback){
  ...
  const sum = numberList.reduce((accum, currentVal) => accum + currentVal, 0)
  return callback(sum);
}
...
function processContains(item, list, callback) {
  ...
  // return callback(container.length > 0);
  ...
  // else return callback(false)
    ...
    // return list.callback(list.includes(item))
...
function processDuplicateFree(list, callback) {
  const noDupes = list.filter(function(item, index, inputArray){
...
function getFullNames(runners) {
  ...
  runners.forEach((value)=>
    namesArray.push(`${value.last_name}, ${value.first_name}`)
  );
  return namesArray;
  
  // let names = [];
  // runners.forEach(function (value){
  //   return names.push(`${value.last_name}, ${value.first_name}`)
  // });
  // return names;
}
...
function firstNamesAllCaps(runners) {
  let allCapsArray = [];
  runners.map((element)=>
    allCapsArray.push(element.first_name.toUpperCase())
  );
  return allCapsArray;
}

  // runners.map(function(value1){
  //   return allCapsArray.push(`${value1.first_name}`)
  // });
  // return allCapsArray.toUpperCase();
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  // const runnersWithTSize = runners.filter(tShirtSize) => {
...
function tallyUpDonations(runners) {
  ...
  // const donationsSum = runners.reduce((accum, currentVal) =>  accum + currentVal, 0)
  // return donationsSum;
}
